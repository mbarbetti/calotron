...
class EuclideanWeightedError(BaseLoss):
    ...
    def transformer_loss(
        ...
        source_coords = tf.tile(source_true[:, None, :, :2], (1, tf.shape(target_true)[1], 1, 1))
        target_coords = tf.tile(target_true[:, :, None, :2], (1, 1, tf.shape(source_true)[1], 1))
        match_distance = tf.norm(target_coords - source_coords, ord="euclidean", axis=-1)
        match_distance = tf.reduce_min(match_distance, axis=-1)
        weights = self._max_match_distance / tf.math.maximum(match_distance, self._max_match_distance)
        if sample_weight is not None:
            weights = tf.math.multiply(weights, sample_weight)
        
        # Regression loss computation
        target_true_ = tf.tile(target_true[:, :, None, :], (1, 1, tf.shape(target_pred)[1], 1))
        target_pred_ = tf.tile(target_pred[:, None, :, :], (1, tf.shape(target_true)[1], 1, 1))
        errors = tf.norm(target_pred_ - target_true_, ord="euclidean", axis=-1)
        errors = tf.reduce_min(errors, axis=-1)
        reg_loss = tf.reduce_sum(tf.math.multiply(weights, errors)) / tf.reduce_sum(weights)
