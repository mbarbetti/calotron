...
class JSDivergence(BaseLoss):
  ...
  def discriminator_loss(self, discriminator, target_true, 
    ...
    return -self._js_div(y_true, y_pred, sample_weight=sample_weight)   # divergence maximization

  def transformer_loss(self, discriminator, target_true, 
                       target_pred, sample_weight=None):
    y_true = discriminator(target_true)
    y_pred = discriminator(target_pred)
    return self._js_div(y_true, y_pred, sample_weight=sample_weight)   # divergence minimization
  ...
  def _js_div(self, y_true, y_pred, sample_weight=None):
    ...
    loss = 0.5 * self._kl_div(y_true, 0.5 * (y_true + y_pred),
